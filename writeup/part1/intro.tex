...
These functions make it straightforward to model the event schedule of a clinical trial and parallelize computation across patients.
...
Secondly, Stan offers efficient inference algorithms, most notably its adaptive \textit{Hamiltonian Monte Carlo} sampler, a gradient-based Markov chains Monte Carlo algorithm \cite{Betancourt:2018, Hoffman:2014}, automatic differentiation variational inference \cite{author:0000}, and penalized maximum likelihood estimators.
...
\end{equation*}
Typically we can evaluate the joint density in the nominator but not the normalizing constant in the denominator.
%
A useful method must therefore be able to generate samples using the \textit{unnormalized} posterior density.
