...
Our prior should allocate probability mass to every plausible parameter value and exclude patently absurd values, e.g. the volume of the central compartment is on the order of ten liters, but it cannot be the size of the Sun.
...
The following statement increments \texttt{target} using the prior on $\sigma$, which a normal density, truncated at 0 to only put mass on positive values.
\begin{lstlisting}
...
The likelihood statement involves terms which we have not defined yet, notably \texttt{logCObs} and \texttt{concentrationObs}.
...
Appendix~\ref{app:name} contains the full Stan code.
...
such as the number of sampling and warmup iterations, and the number of chains and whether to run them in parallel.
\begin{lstlisting}
...
For applications in pharmacometrics, we recommend specifying the initial starting points, for example by drawing them from the priors.
The full R script can be found in the Supplementary Material.

The \texttt{fit} object stores the samples generated by HMC from which can deduce the sample mean, sample variance, and sample quantiles of our posterior distribution.
All this information is accessible using \texttt{fit\$summary()} and summarized in table~\ref{tab:summary}.
Naturally we can construct more results, such as posterior covariances, using our posterior samples.
...
$CL$    &    10.0  &  10.0  &  0.378  & 0.367 &  9.39 &  10.6  &  1.00   & 1580  &  1348 \\\rowcolor[gray]{0.95} $Q$      &  19.8    & 19.5  &  4.00  &  4.01 &  13.8  &   26.8 &   1.00  &   985 &    1235 \\$V_\mathrm{cent}$   &   41.2  &  40.8 &   9.71   & 9.96  & 25.6 &   57.7   & 1.00   &  732  &  1120 \\\rowcolor[gray]{0.95} $V_\mathrm{peri}$    &  124 &   123 &    18.0  &  18.0 &    97.1 &  155 &   1.00  &  1877 &    1279 \\$k_a$       &  1.73   & 1.67 &  0.523  & 0.522   & 1.01 &   2.68 &   1.00   &  762 &    1108 \\\rowcolor[gray]{0.95} $\sigma$   & 0.224 &   0.222 &  0.0244 &  0.0232 &  0.187 &   0.269 &  1.01  & 1549 &   1083
  \end{tabular}
  \caption{Summary of results when fitting a two compartment model. \textit{The first columns return the estimates of the posterior mean, median, standard deviation, median absolute deviation, $5^\mathrm{th}$ and $95^\mathrm{th}$ quantiles, based on our approximate samples.
...
We should naturally not assume that our inference is reliable.
There is no guarantee that a particular inference algorithm will work across all the applications we will encounter.
...
\end{equation*}
%
a quantity also termed the expectation value.
...
\end{equation*}
%
This is a powerful result for two reasons: first it tells us that our estimator is unbiased and more importantly that the expected error is driven by the variance of $f$ divided by our sample size, $n$.

\subsubsection{Checking for bias with $\hat R$}

An MCMC method, in general, will not verify these conditions.
MCMC samples are biased for several reasons.
First, the Markov chain is initialized at a point set by the modeler, not a point drawn from the posterior.
The samples we generate are correlated to one another, meaning any sample will have some correlation to the starting point.
If we run the algorithm for a long enough time, the correlation to the initial point becomes negligible and the chain ``forgets'' its starting point.
...
To identify this bias, we may run multiple Markov chains, each started at different points, and check that they all convergence to the same area.
...
\begin{figure}
  ...
  \caption{Trace plots. \textit{The sampled values for each parameters are plotted against the iterations during the sampling phase. Multiple Markov chains were initialized at different points. However, once in the sampling phase, we cannot distinguish them, as shown by the ``fuzzy caterpillar'' shapes.}}
  \label{fig:trace}
  \end{center}
\end{figure}

If $\hat R \gg 1$ or if the chains were not mixing, this would be cause for concern and an invitation to adjust our inference method.
...
Let us assume now that our samples are indeed unbiased.
The expected error of our estimator is now determined by the variance.
Under certain regularity conditions, our estimator follow an MCMC central limit theorem,
\begin{equation*}
...
This change accounts for the fact are samples are not independent: their correlation induces a loss in information, which increase the variance of our estimator.
...
Or we may evaluate at the models ability to perform a certain task, e.g. classification or prediction, as is often done in machine learning.
...
\begin{figure}
  ...
  \end{center}
  \caption{Posterior predictive checks for one compartment model. \textit{The circles represent the observed data and the shaded areas the $50^\mathrm{th}$ and $90^\mathrm{th}$ credible intervals based on posterior draws. A graphical inspection suggests the credible interval is wider than for the one compartment model than they are for the two compartment model.}}
...
Here $x_i$ denote the covariate and in our example, the relevant row in the event schedule.
We denote this reduced data set, $y_{-i}$.
We then generate a prediction $(\tilde y_i, x_i)$ using the fitted model, and compare $\tilde y_i$ to $y_i$.
One metric to make this comparison is the squared error, $(\tilde y_i - y_i)^2$.
...
for (i in 1:nObs)
  log_lik[i] =  
    normal_lpdf(logCObs[i] | log(concentrationObs[i]), 
                sigma);
\end{lstlisting}
These results can be extracted and fed into Loo to compute $\mathrm{elp}_\mathrm{loo}$.
